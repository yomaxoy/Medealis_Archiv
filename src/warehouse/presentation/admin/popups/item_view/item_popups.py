...
def show_item_confirmation_popup(item_data: Dict[str, Any]):
    ...
    edited_notes = st.text_area(
        "Notizen:", value=item_data.get("notes", ""), key="popup_item_notes"
    )
...
def show_item_edit_popup(item_data: Dict[str, Any]):
    ...
    with col_header2:
        ...
        if all(
            ...
            render_open_folder_button(
                item_data, key_suffix="edit_popup", button_type="secondary"
            )
        elif (
            item_data.get("article_number")
            and item_data.get("batch_number")
            and item_data.get("delivery_number")
        ):
    ...
    with col2:
        quantity = st.number_input(
            "Quantity:", value=item_data.get("quantity", 1), min_value=1
        )
        employee_name = st.text_input(
            "Employee Name:", value=item_data.get("employee_name", "")
        )
        status = st.selectbox(
            "Status:",
            ...
            else ["PENDING", "IN_PROGRESS", "COMPLETED", "FAILED"].index(
    ...
    with st.expander("📝 Erweiterte Informationen", expanded=False):
        ...
        with col3:
            designation = st.text_input(
                "Bezeichnung:", value=item_data.get("designation", "")
            )
            revision_number = st.text_input(
                "Revision Number:",
                value=str(item_data.get("revision_number", ""))
                if item_data.get("revision_number")
                else "",
            )
            drawing_reference = st.text_input(
                "Drawing Reference:", value=item_data.get("drawing_reference", "")
            )

        with col4:
            storage_location = st.text_input(
                "Lagerplatz:", value=item_data.get("storage_location", "")
            )
            manufacturer = st.text_input(
                "Kompatibilität:",
                value=item_data.get("kompatibilitaet", "")
                or item_data.get("manufacturer", ""),
            )
    ...
    description = st.text_area("Beschreibung:", value=item_data.get("description", ""))

    notes = st.text_area("Notizen:", value=item_data.get("notes", ""))
...
def show_item_delete_popup(item_data: Dict[str, Any]):
    ...
    st.info(
        "💡 **Hinweis:** Prüfen Sie ob das Item in anderen Prozessen verwendet wird."
    )
...
def show_item_details_popup(item_data: Dict[str, Any]):
    ...
    technical_data = [
        (field, label) for field, label in technical_fields if item_data.get(field)
    ]
    ...
    try:
        ...
        if "delivery" in services:
            ...
            if hasattr(delivery_service, "get_delivery"):
                delivery = delivery_service.get_delivery(
                ...
                if delivery:
                    ...
                    with col2:
                        dd = delivery.get("delivery_date", "N/A")
                        st.write(f"**Delivery Date:** {dd}")
                    with col3:
...
def show_batch_tracking_popup(item_data: Dict[str, Any]):
    ...
    try:
        ...
        batch_items = [
            item for item in all_items if item.get("batch_number") == batch_number
        ]
        ...
        st.write(
            f"**{len(batch_items)} Items** gefunden mit Batch Number: `{batch_number}`"
        )

        # Show batch items
        for i, batch_item in enumerate(batch_items):
            with st.expander(
                f"Item {i+1}: {batch_item.get('article_number', 'N/A')}",
                expanded=(i == 0),
            ):
                col1, col2, col3 = st.columns(3)
                ...
                with col1:
                    ...
                    st.write(
                        f"**Delivery:** {batch_item.get('delivery_number', 'N/A')}"
                    )
        ...
        with col4:
            completed_items = len(
                [item for item in batch_items if item.get("status") == "COMPLETED"]
            )
