...
from warehouse.application.services.document_operations.document_opening_service import document_opening_service
...
def render_open_folder_button(
    ...
    required_fields = ["article_number", "batch_number", "delivery_number", "supplier_name"]
    ...
    try:
        ...
        context = StorageContextData(
            ...
            manufacturer=manufacturer,  # Verwende abgeleiteten Manufacturer
            context_source="ui"
        )
        ...
        if use_server_path:
            ...
            result = path_resolver.resolve_server_storage_path(context, create_folders=False)
            ...
            with col1:
                ...
                if st.button("📁 Ordner erstellen", key=create_key, type="secondary"):
                    ...
                    if result.success:
                        ...
                        st.error(f"❌ Fehler beim Erstellen des Ordners: {result.error}")
...
def render_folder_action_with_status(
    ...
    Zeigt visuell an ob Ordner bereits existiert (grün) oder noch erstellt werden muss (gelb).
