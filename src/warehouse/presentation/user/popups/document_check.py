...
class DocumentCheckPopup(InspectionPopup):
    ...
    def render_body(self) -> Dict[str, Any]:
        ...
        st.caption("*Bestätigen Sie, dass die erstellten Dokumente korrekt angebracht bzw. beigelegt wurden:*")
        ...
        form_data['materialzeugnis'] = st.session_state.get('_temp_materialzeugnis', False)
        form_data['messprotokolle'] = st.session_state.get('_temp_messprotokolle', False)
        form_data['beschichtungszeugnis'] = st.session_state.get('_temp_beschichtungszeugnis', False)
        form_data['haertezeugnis'] = st.session_state.get('_temp_haertezeugnis', False)
        form_data['weitere_zeugnisse'] = st.session_state.get('_temp_weitere_zeugnisse', '')
        form_data['label_attached'] = st.session_state.get('_temp_label_attached', False)
        ...
        if not label_attached or not slip_attached:
            st.info("💡 Bitte bestätigen Sie, dass Label und Begleitschein angebracht/beigelegt wurden.")
    ...
    def render_footer(self) -> Optional[str]:
        ...
        return render_standard_footer(
            save_label="✅ Dokumente bestätigen",
    ...
    def handle_primary_action(self, form_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.info(f"🔥 USER POPUP: Employee name from form_data: '{employee_name}'")
            ...
            # Validierung - CORRECT KEYS from render_body()!
            if not st.session_state.get('doc_label_attached', False):
                st.error("❌ Label muss angebracht sein!")
                return

            if not st.session_state.get('doc_slip_attached', False):
                st.error("❌ Begleitschein muss beigelegt sein!")
                return
            ...
            accompanying_document_attached = st.session_state.get('doc_slip_attached', False)

            # Log for debugging (ADMIN STYLE)
            logger.info(f"🔍 STEP 2 DEBUG: Documents found: {documents_found}")
            logger.info(f"🔍 STEP 2 DEBUG: Physical documents: Label={label_attached}, Begleitschein={accompanying_document_attached}")
            ...
            if uploaded_docs:
                st.write(f"📤 Speichere {len(uploaded_docs)} hochgeladene Dokument(e)...")
                try:
            ...
            try:
                ...
                item_data_for_pdb = fresh_item_data if fresh_item_data else self.item_data
            ...
            try:
                ...
                logger.info(f"🔥 WORKFLOW COMPLETE: Certificates dict: {certificates_dict}")
