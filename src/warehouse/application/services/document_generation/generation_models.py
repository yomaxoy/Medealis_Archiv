...
class GenerationContext:
    ...
    document_type: 'DocumentType' = None           # Wird aus document_types.py importiert
    generation_timestamp: datetime = field(default_factory=datetime.now)
    template_version: str = "1.0"

    # FLEXIBLE DATEN
    placeholders: Dict[str, Any] = field(default_factory=dict)  # Zusätzliche Placeholder
    custom_data: Dict[str, Any] = field(default_factory=dict)   # Dokument-spezifische Daten
    ...
    def to_placeholder_dict(self) -> Dict[str, str]:
        ...
        try:
            ...
            placeholder_dict = {
                ...
                'article_number': self.article_number,  # Added for barcode template compatibility
                ...
                'orderdate': self.delivery_date,   # Für Begleitschein Template (fallback)
                ...
                # Zertifikat-Platzhalter (Standard: alle leer, werden durch custom_data überschrieben)
                ...
                # PDB-spezifische Mengen-Platzhalter (Standard-Werte, werden durch custom_data überschrieben)
                'orderq': '-',         # FIXED: "-" statt '0' wenn keine Bestellmenge
                'deliveryq': str(self.quantity) if self.quantity else '-', # FIXED: "-" wenn keine Liefermenge
                'order_quantity': '-', # FIXED: "-" statt '0'
                'delivery_quantity': str(self.quantity) if self.quantity else '-', # FIXED: "-" wenn keine Liefermenge
            ...
            if self.placeholders:
                placeholder_dict.update({k: str(v) for k, v in self.placeholders.items()})
            ...
            if self.custom_data:
                for key, value in self.custom_data.items():
                    ...
                    if isinstance(value, bool):
                        ...
                        placeholder_dict[key] = value.strftime("%d.%m.%Y") if hasattr(value, 'strftime') else str(value)
                    else:
                        placeholder_dict[key] = str(value) if value is not None else "-"  # FIXED: "-" statt ""

            # Spezial-Fall: qty sollte delivery_quantity entsprechen, falls verfügbar
            if 'delivery_quantity' in placeholder_dict and placeholder_dict['delivery_quantity'] != '0':
    ...
    def validate_for_document_type(self, document_type: 'DocumentType') -> 'ValidationResult':
        """
        ...
        try:
            ...
            if document_type:
                required_fields = self._get_required_fields_for_document_type(document_type)

                for field_name in required_fields:
                    field_value = getattr(self, field_name, None)
                    if not field_value or (isinstance(field_value, str) and not field_value.strip()):
                        validation_result.add_error(f"{field_name} ist erforderlich für {document_type.value}")

            # Completeness Score Warnung
            if self.completeness_score < 0.7:
                validation_result.add_warning(f"Niedrige Datenqualität ({self.completeness_score:.1%})")
    ...
    def _get_required_fields_for_document_type(self, document_type: 'DocumentType') -> List[str]:
        ...
        field_requirements = {
            ...
            DocumentType.WARENEINGANGSKONTROLLE: ['article_number', 'batch_number', 'delivery_date'],
            DocumentType.INCOMING_GOODS_INSPECTION: ['article_number', 'batch_number', 'delivery_number']
        }

        return field_requirements.get(document_type, ['batch_number', 'delivery_number'])

    def merge_with_storage_context(self, storage_context: 'StorageContextData') -> 'GenerationContext':
        """
        ...
        try:
            ...
            merged_context = GenerationContext(
                ...
                completeness_score=max(self.completeness_score, storage_context.completeness_score)
            )
...
class GenerationResult:
    ...
    def get_summary(self) -> str:
        ...
        if self.success:
            ...
            return (f"✅ {self.document_type.value if self.document_type else 'Document'} "
...
class BatchGenerationResult:
    ...
    def get_summary(self) -> str:
        ...
        if self.overall_success:
            return (f"✅ Batch completed: {self.successful_documents}/{self.total_documents} "
                   f"documents in {self.total_generation_time:.2f}s")
        else:
            return (f"⚠️ Batch partial: {self.successful_documents}/{self.total_documents} "
    ...
    def get_detailed_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "statistics": {
                ...
                "average_generation_time": self.total_generation_time / max(self.total_documents, 1)
            },
            "successful_documents": [
                {"type": r.document_type.value if r.document_type else "unknown",
                 "path": str(r.document_path), "time": r.generation_time}
