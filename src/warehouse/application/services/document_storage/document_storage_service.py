...
from .storage_context import StorageContext, StorageContextData, storage_context
from .path_resolver import PathResolver, PathResult, path_resolver
from .storage_validator import StorageValidator, ValidationResult, storage_validator, ValidationLevel
...
class DocumentStorageService:
    ...
    def __init__(self, validation_level: ValidationLevel = ValidationLevel.STANDARD):
        ...
        self.logger.info(f"DocumentStorageService initialized with validation level: {validation_level.value}")
        self.logger.info(f"Storage mode: {self.storage_mode}")
        self.logger.info(f"Server storage: {'enabled' if self.use_server_storage else 'disabled'}")
        self.logger.info(f"SharePoint storage: {'enabled (fallback)' if self.use_sharepoint else 'disabled'}")
    ...
    def save_document(
        ...
        **additional_context
        ...
        """
        try:
            self.logger.info(f"Saving document: {document_name} (type: {document_type})")
            ...
            context_validation = self.storage_validator.validate_storage_context(context)
            if not context_validation.is_valid:
                return StorageResult(
                    success=False,
                    error=f"Invalid storage context: {'; '.join(context_validation.errors)}",
            ...
            if not doc_validation.is_valid:
                return StorageResult(
                    success=False,
                    error=f"Document validation failed: {'; '.join(doc_validation.errors)}",
                    warnings=doc_validation.warnings
                )

            # 4. FILENAME SANITIZATION - Bereinige Dateiname
            ...
            perm_validation = self.storage_validator.validate_storage_permissions(path_result.path)
            ...
            primary_save_attempted = False
            primary_save_success = False

            try:
                primary_save_attempted = True
                with open(final_file_path, 'wb') as f:
                    f.write(document_data)
                primary_save_success = True
                self.logger.info(f"Document saved to {storage_location}: {final_file_path}")
            ...
            if self.use_sharepoint and self.sharepoint_client.is_available() and not primary_save_success:
                try:
                    sharepoint_upload_attempted = True
                    self.logger.warning(f"Primary storage failed, attempting SharePoint fallback...")
                    ...
                    self.logger.info(f"Uploading to SharePoint (fallback): {sharepoint_path}/{safe_filename}")
                    ...
                    if upload_result.success:
                        ...
                        self.logger.info(f"✅ SharePoint fallback successful: {sharepoint_url}")
                    else:
                        error_msg = f"SharePoint fallback failed: {upload_result.error}"
            ...
            result.metadata.update({
                ...
                'final_storage': 'server' if (storage_location == 'server' and primary_save_success)
                               else 'local' if (storage_location == 'local' and primary_save_success)
                               else 'sharepoint_fallback' if sharepoint_url
                               else 'unknown',
                'server_unavailable_info': server_unavailable_info  # NEU: Für GUI-Warnung
            })

            # Log final status
            if primary_save_success and not sharepoint_url:
                self.logger.info(f"✅ Document saved to {storage_location.upper()}: {final_file_path}")
            elif primary_save_success and sharepoint_url:
                self.logger.info(f"✅ Document saved to {storage_location.upper()} + SharePoint backup: {final_file_path}")
            elif sharepoint_url:
                self.logger.warning(f"⚠️  Primary storage failed, document saved to SharePoint fallback: {sharepoint_url}")
    ...
    def validate_document_before_storage(
        ...
        try:
            ...
            doc_validation = self.storage_validator.validate_document_data(document_data, filename)
            context_validation = self.storage_validator.validate_storage_context(context)

            # Erstelle kombiniertes Ergebnis
            combined_result = ValidationResult(is_valid=doc_validation.is_valid and context_validation.is_valid)
    ...
    def clear_cache(self) -> Dict[str, int]:
        ...
        logger.info(f"Cleared {cleared_methods} method caches in DocumentStorageService")
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        if hasattr(self.validate_document_before_storage, 'cache_stats'):
            stats['document_validation'] = self.validate_document_before_storage.cache_stats()
    ...
    def save_delivery_slip_pdf(
        ...
        try:
            ...
            if self.use_server_storage and PathLib("A:\\").exists():
                ...
                storage_location = "server"
                self.logger.info(f"Using SERVER storage for delivery slip (primary)")
            else:
            ...
            self.logger.info(f"Path resolution result: success={path_result.success}, path={path_result.path}")
            ...
            try:
                ...
                primary_save_success = True
                self.logger.info(f"✅ Delivery slip saved to {storage_location.upper()}: {final_file_path}")
                self.logger.info(f"Delivery slip successfully written! Size: {len(document_data)} bytes")

            except Exception as e:
                error_msg = f"Failed to save delivery slip to {storage_location}: {str(e)}"
                self.logger.error(error_msg)
                # Speicher-Fehler ist kritisch, wenn kein SharePoint-Fallback verfügbar
                if not self.use_sharepoint:
                    return StorageResult(success=False, error=error_msg)
            ...
            if self.use_sharepoint and self.sharepoint_client.is_available() and not primary_save_success:
                try:
                    sharepoint_upload_attempted = True
                    self.logger.warning(f"Primary storage failed for delivery slip, attempting SharePoint fallback...")
                    ...
                    self.logger.info(f"Uploading delivery slip to SharePoint (fallback): {sharepoint_path}/{filename}")
                    ...
                    if upload_result.success:
                        sharepoint_url = upload_result.file_url
                        self.logger.info(f"✅ Delivery slip SharePoint fallback successful: {sharepoint_url}")
                    else:
                        error_msg = f"SharePoint fallback failed: {upload_result.error}"
                        sharepoint_warnings.append(error_msg)
                        self.logger.error(error_msg)
            ...
            if not primary_save_success and not sharepoint_url:
                return StorageResult(
                    success=False,
                    error="All storage methods failed for delivery slip (server/local + SharePoint)",
                    warnings=sharepoint_warnings
                )
            ...
            if primary_save_success and not sharepoint_url:
                ...
                self.logger.warning(f"⚠️  Primary storage failed, delivery slip saved to SharePoint fallback: {sharepoint_url}")
    ...
    def get_delivery_slip_path(
        ...
        try:
            ...
            if not path_result.success:
                self.logger.warning(f"Delivery slip path resolution failed for {supplier_name}: {path_result.error}")
                return Path("."), [path_result.error or "Unknown path resolution error"]
    ...
    def _scan_delivery_slip_folder(self, folder_path: Path, supplier_name: str) -> List[Dict[str, Any]]:
        """
            ...
            for file_path in folder_path.iterdir():
                if file_path.is_file() and file_path.suffix.lower() in ['.pdf', '.jpg', '.jpeg', '.png', '.tiff']:
                    ...
                    delivery_slip_info = {
                        ...
                        'folder_year': folder_path.parent.name if folder_path.parent.name.isdigit() else None,
                        'folder_month': folder_path.name
                    }

                    # Versuche Lieferscheinnummer aus Dateiname zu extrahieren
                    delivery_number = self._extract_delivery_number_from_filename(file_path.name)
                    if delivery_number:
    ...
    def get_documents_for_merge(
        ...
        Note:
            Sets self.last_merge_source to 'server', 'local', or 'sharepoint' to indicate document source
        """
        import tempfile
        ...
        try:
            ...
            if self.use_server_storage and PathLib("A:\\").exists():
                ...
                self.logger.info(f"[MERGE] PRIMARY: Checking server storage for merge documents: {batch_number}")
            else:
                # LOKALE Speicherung (Primär-Fallback)
                path_result = self.path_resolver.resolve_storage_path(context, create_folders=False)
                primary_source = "local"
                self.logger.info(f"[MERGE] PRIMARY: Checking local storage for merge documents: {batch_number}")
            ...
            if path_result.success and path_result.path.exists():
                ...
                self.logger.info(f"[MERGE] Found {len(local_pdfs)} PDFs in {primary_source} storage")
                ...
                for local_pdf in local_pdfs:
                    ...
                    self.logger.info(f"[MERGE] ✓ Loaded from {primary_source}: {local_pdf.name}")
            ...
            if not downloaded_files and self.use_sharepoint and self.sharepoint_client.is_available():
                self.logger.warning(f"[MERGE] FALLBACK: No files in primary storage, trying SharePoint download: {batch_number}")

                # Bestimme SharePoint-Pfad
                sharepoint_path = self.path_resolver.resolve_sharepoint_path(
                    document_type="pdb",  # Verwende einen Standard-Typ für Pfad-Auflösung
                    context=context
                )
                ...
                self.logger.info(f"[MERGE] Starting parallel download of {len(pdf_files)} PDF files...")

                def download_single_file(file_info):
                    """Download eine einzelne Datei (für parallel execution)"""
                    filename = file_info.get('name', '')
                    try:
                        self.logger.info(f"[MERGE] Downloading {filename} from SharePoint...")
                        ...
                        if file_bytes:
                            ...
                            with open(temp_file, 'wb') as f:
                                f.write(file_bytes)
                            self.logger.info(f"[MERGE] ✓ Downloaded from SharePoint: {filename} ({len(file_bytes)} bytes)")
                            return temp_file
                        else:
                            self.logger.warning(f"[MERGE] ✗ Failed to download from SharePoint: {filename}")
                            return None
                    except Exception as e:
                ...
                with ThreadPoolExecutor(max_workers=5) as executor:
                    futures = {executor.submit(download_single_file, f): f for f in pdf_files}
                ...
                self.logger.info(f"[MERGE] SharePoint fallback download complete: {len(downloaded_files)} files")
                if downloaded_files:
                    self.last_merge_source = "sharepoint"  # Mark as SharePoint fallback source
            else:
                self.logger.info(f"[MERGE] SharePoint not available or disabled, no fallback available")

            self.logger.info(f"Total documents ready for merge: {len(downloaded_files)}")
